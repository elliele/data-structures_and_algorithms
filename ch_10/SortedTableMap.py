...
class SortedTableMap(MapBase):
    """Map implementaiton using a sorted table."""
    ...
    def find_gt(self, k):
        """Return (key, value) pair with least strictly greater than k."""
        j = self._find_index(k, 0, len(self._table) - 1)                # j's key >= k
        if j < len(self._table) and self._table[j]._key == k:
            j += 1                                                      # advanced past match
        if j < len(self._table):
            return (self._table[j]._key, self._table._value)
        else:
            return None
    ...
    def find_range(self, start, stop):
        ...
        while j < len(self._table) and (stop is None or self._table[j]._key < stop):
            yield (self._table._key, self._table._value)
            j += 1
